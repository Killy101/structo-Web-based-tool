...
def _extract_pattern_lines(rule: str) -> list[str]:
    """
    Extract explicit regex strings from a citation rule text block.
    Delegates to brd_data._extract_explicit_patterns — single implementation.
    """
    from src.services.brd_data import _extract_explicit_patterns
    return _extract_explicit_patterns(rule)
...
def _sanitize_path_transform_output(metajson: dict[str, Any]) -> None:
    ...
    if isinstance(raw_pt, dict):
        for key, value in raw_pt.items():
            ...
            elif rows == [] and level_key not in pt:
                pt[level_key] = {"patterns": [], "case": str(value.get("case") or "")}
    ...
    if isinstance(raw_lp, dict):
        for key, pats in raw_lp.items():
            ...
            if rows_out:
                pt[level_key] = {"patterns": rows_out, "case": ""}

    metajson["pathTransform"] = pt


# ── /generate/metajson helpers ────────────────────────────────────────────────
...
def _build_levels_from_toc(
    ...
    """
    Merge TOC sections with citation rules into a flat level list.

    When the payload arrives at /generate/metajson it has already been through
    brd_to_metajson_input(), so the merging done in brd_data._build_levels()
    is already reflected in the toc + citations dicts.  This function handles
    the final re-merge for the assembler's input format.
    """
    if not toc:
        return []
...
def _normalise_metadata(metadata: dict[str, Any] | None, format_: str) -> dict[str, Any]:
    """
    Map raw metadata keys → canonical metajson schema keys.

    When the payload comes from brd_data (via brd_to_metajson_input) the
    metadata dict already uses canonical keys like "Content Category Name".
    This function handles both that case AND the legacy camelCase keys sent
    ...
    # If brd_data already produced canonical keys, pass them through directly.
    if "Content Category Name" in metadata or "Source Name" in metadata:
        result = dict(metadata)
        result.pop("_format", None)
...
async def process_document(
    ...
    try:
        ...
        # ── Image extraction ──────────────────────────────────────────────────
        # Images are already extracted inside extract_brd() when brd_id is set.
        # Here we only need to persist them to the DB via Node.js.
        image_metadata: list[dict] = result.get("cell_images") or []

        if brd_id and suffix == ".docx" and image_metadata:
            print(f"[DEBUG] Persisting {len(image_metadata)} images to DB for brd_id={brd_id}")
            try:
                node_base = os.environ.get("NODE_API_URL", "http://localhost:4000")
                resp = httpx.post(
                    f"{node_base}/brd/{brd_id}/images",
                    json={"images": image_metadata},
                    timeout=30,
                )
                print(f"[DEBUG] Saved {len(image_metadata)} images to DB: {resp.status_code}")
            except Exception as save_err:
                print(f"[WARN] Could not persist images to DB: {save_err}")
        else:
            print(f"[DEBUG] Skipping image DB persist: brd_id={brd_id}, "
                  f"suffix={suffix}, images={len(image_metadata)}")
...
async def build_level_patterns(payload: LevelPatternRequest):
    ...
    level_numbers = [
        n for n in (_parse_level_number(c.level) for c in citations)
        if n is not None
    ]
    ...
    inferred = (
        generate_level_patterns(language=language, levels=inferred_input)
        if inferred_input else {}
    )
    ...
    # Explicit regex in citation text overrides everything else.
    # Uses brd_data._extract_explicit_patterns — one implementation.
    for citation in citations:
        level = _parse_level_number(citation.level)
        if level is None:
            continue
...
async def generate_metajson(payload: GenerateMetajsonRequest):
    ...
    # Strip stale pathTransform and levelPatterns from brdConfig so the
    # language cleanup rules in english.py always take effect.
    # These keys come from a previously-saved metajson being round-tripped
    # as brdConfig — keeping them causes old rules to win over the fresh
    # ENGLISH_PATH_TRANSFORM_CLEANUP loaded from the language file.
    brd_config_clean: dict | None = None
    if payload.brdConfig:
        brd_config_clean = {
            k: v for k, v in payload.brdConfig.items()
            if k not in ("pathTransform", "path_transform",
