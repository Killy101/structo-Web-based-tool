...
const ExportIcon  = () => <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>;
const RestoreIcon = () => <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6"/></svg>;
...
export default function BrdPage() {
  ...
  const fetchDeletedBrds = useCallback(async () => {
    try {
  ...
  const confirmRestore = async () => {
    ...
    try {
      await api.post(`/brd/${restoreTarget.id}/restore`);
      setDeletedBrds(prev => prev.filter(b => b.id !== restoreTarget.id));
  ...
  return (
    <div className="relative h-full w-full min-h-0 bg-slate-50 dark:bg-slate-950 flex flex-col">
      ...
      <div className="flex flex-wrap items-center gap-2 px-4 py-2.5 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800">
        ...
        {/* Trash (deleted BRDs) */}
        <button onClick={handleShowDeleted} className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-xs font-medium text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
          <TrashIcon /> Trash
        </button>
                ...
                <tr key={brd.id}
                  ...
                  <td className="px-3 py-3 whitespace-nowrap text-center">
                    <div className="flex items-center justify-center gap-0.5">
                      ...
                      <button onClick={() => handleRemove(brd)} title="Move to Trash"
              ...
              <div className="p-6">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-50">Move BRD to Trash?</h3>
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 leading-relaxed">
                      <span className="font-semibold text-slate-700 dark:text-slate-200">{deleteTarget.id}</span> will be moved to Trash. You can restore it later from the Trash view.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2.5 mt-5">
                  ...
                  <button onClick={confirmDelete} disabled={deleteLoading}
                    ...
                    {deleteLoading
                      ? <span className="flex items-center justify-center gap-1.5"><svg className="w-3.5 h-3.5 animate-spin" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"/></svg>Moving…</span>
                      : "Move to Trash"}
                  </button>
                </div>
              </div>
              ...
              <div className="p-6">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-50">Move {selected.size} BRD{selected.size > 1 ? "s" : ""} to Trash?</h3>
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 leading-relaxed">All selected documents will be moved to Trash. You can restore them later from the Trash view.</p>
                  </div>
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-2xl z-10">
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
              ...
              <div className="px-6 pt-5 pb-4 flex items-center justify-between border-b border-slate-200 dark:border-slate-800">
                <div>
                  <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-50 flex items-center gap-2">
                    <TrashIcon /> Trash
                  </h3>
                  <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">Soft-deleted BRDs — restore to bring them back</p>
                </div>
                <button onClick={() => setShowDeleted(false)} className="text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                  <CloseIcon />
                </button>
              </div>
              <div className="p-5 max-h-96 overflow-y-auto">
                {deletedLoading ? (
                  ...
                  <div className="text-center py-10 text-slate-400 dark:text-slate-500 text-xs">No deleted BRDs found.</div>
                ) : (
                  <table className="w-full text-xs border-collapse">
                    <thead>
                      <tr className="bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
                        <th className="px-3 py-2 text-left text-[10px] font-semibold text-slate-500 uppercase tracking-wider">BRD ID</th>
                        <th className="px-3 py-2 text-left text-[10px] font-semibold text-slate-500 uppercase tracking-wider">Title</th>
                        <th className="px-3 py-2 text-left text-[10px] font-semibold text-slate-500 uppercase tracking-wider">Deleted</th>
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                      {deletedBrds.map(b => (
                        <tr key={b.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                          <td className="px-3 py-2.5 font-mono text-[11px] text-blue-600 dark:text-blue-400">{b.id}</td>
                          <td className="px-3 py-2.5 text-slate-700 dark:text-slate-200 max-w-[220px] truncate">{b.title}</td>
                          <td className="px-3 py-2.5 text-slate-400 dark:text-slate-500">{b.deletedAt}</td>
                          <td className="px-3 py-2.5 text-center">
                            <button onClick={() => { setRestoreTarget(b); setShowDeleted(false); }}
                              className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-[11px] font-medium text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800/50 hover:bg-emerald-100 dark:hover:bg-emerald-900/40 transition-colors">
                              <RestoreIcon /> Restore
                            </button>
                          </td>
                        </tr>
      ...
      {restoreTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/50 backdrop-blur-sm" onClick={() => !restoreLoading && setRestoreTarget(null)} />
          <div className="relative w-full max-w-md z-10">
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
              <div className="h-1 w-full bg-gradient-to-r from-emerald-400 to-teal-500" />
              <div className="p-6">
                <div className="flex items-start gap-4">
                  <div className="w-11 h-11 rounded-full bg-emerald-50 dark:bg-emerald-900/20 flex items-center justify-center flex-shrink-0 border border-emerald-100 dark:border-emerald-800/40">
                    <RestoreIcon />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-50">Restore BRD?</h3>
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 leading-relaxed">
                      <span className="font-semibold text-slate-700 dark:text-slate-200">{restoreTarget.id}</span> will be restored and visible in the BRD registry again.
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-2.5 mt-5">
                  <button onClick={() => setRestoreTarget(null)} disabled={restoreLoading}
                    className="flex-1 py-2.5 rounded-xl text-xs font-semibold text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 disabled:opacity-50 transition-colors">
                    Cancel
                  </button>
                  <button onClick={confirmRestore} disabled={restoreLoading}
                    className="flex-1 py-2.5 rounded-xl text-xs font-semibold text-white bg-emerald-600 hover:bg-emerald-700 active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed transition-all">
                    {restoreLoading
                      ? <span className="flex items-center justify-center gap-1.5"><svg className="w-3.5 h-3.5 animate-spin" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"/></svg>Restoring…</span>
                      : "Restore BRD"}
                  </button>
                </div>
              </div>
