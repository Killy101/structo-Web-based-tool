...
/** Pick the best display name: sourceName → contentName → title */
...
export default function BrdPage() {
  ...
  const handleRemove = async (id: string) => {
    try {
      ...
      console.error("Failed to delete BRD:", err);
    }
  };
  ...
  return (
    <div className="h-full w-full min-h-0 px-6 py-5 text-xs flex flex-col gap-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3">
        {statCards.map((s) => (
          <div key={s.label} className={`rounded-2xl p-4 flex items-center gap-3.5 bg-gradient-to-br ${s.gradient} border ${s.border} hover:shadow-md transition-shadow`}>
          ...
          <table className="w-full text-xs border-collapse">
            <thead>
              <tr className="bg-slate-100 dark:bg-slate-800/80 border-b border-slate-200 dark:border-slate-700">
                ...
                <th className="px-4 py-3 text-left   font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest text-[10px] whitespace-nowrap">Source / Content Name</th>
              ...
              {loading ? (
                <tr>
                  <td colSpan={7} className="px-4 py-12 text-center text-slate-400 dark:text-slate-500">
                    <div className="flex items-center justify-center gap-2">
                      ...
                      <span className="text-xs font-medium">Loading BRDs…</span>
                    </div>
                  </td>
                </tr>
              ) : filtered.length === 0 ? (
                <tr>
                  <td colSpan={7} className="px-4 py-12 text-center text-slate-400 dark:text-slate-500">
                    ...
                    <div className="font-medium">
                      {brds.length === 0 ? "No BRDs yet — click New BRD to get started." : "No BRDs found — try adjusting your search or filter."}
                    </div>
                  </td>
                </tr>
                ...
                return (
                  ...
                  >
                    ...
                    {/* Last Updated */}
                    <td className="px-4 py-3.5 whitespace-nowrap text-center">
                      <span className="font-mono text-[11px] font-normal text-slate-600 dark:text-slate-500">{brd.lastUpdated}</span>
                    </td>
          ...
          <div className="relative w-full max-w-sm z-10">
            <Card className="shadow-2xl">
              <CardHeader
                ...
                action={
                  <button onClick={() => setHistoryBrd(null)} className="text-slate-500 hover:text-slate-700 dark:hover:text-slate-200 p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                    <CloseIcon />
                  </button>
                }
              />
                  ...
                  <div className="space-y-3">
                    {HISTORY_MOCK(historyBrd).map((h, i) => (
                      <div key={i} className="flex items-start gap-3">
                        <div className={`w-10 h-10 rounded-full flex-shrink-0 flex items-center justify-center border-2 z-10 ${h.latest ? "bg-emerald-50 dark:bg-emerald-900/30 border-emerald-400 dark:border-emerald-600" : "bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700"}`}>
                          {h.latest ? (
                        ...
                        <div className={`flex-1 flex items-center justify-between p-3 rounded-xl border ${h.latest ? "bg-emerald-50/80 dark:bg-emerald-900/10 border-emerald-200 dark:border-emerald-900/40" : "bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700/60"}`}>
                          <div>
                            <div className="flex items-center gap-2 flex-wrap">
                              <span className="font-mono text-xs font-bold text-slate-800 dark:text-slate-300 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 px-2 py-0.5 rounded-md">{h.ver}</span>
                              {h.latest && <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-100 dark:bg-emerald-900/40 text-emerald-800 dark:text-emerald-400 border border-emerald-300 dark:border-emerald-800">Latest</span>}
                            </div>
                            <div className="text-xs font-medium text-slate-600 dark:text-slate-400 mt-1">{h.note}</div>
                          </div>
                          <span className="font-mono text-[10px] font-medium text-slate-500 whitespace-nowrap ml-3">{h.date}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                </div>
                <Button variant="secondary" className="w-full justify-center" onClick={() => setHistoryBrd(null)}>Close</Button>
              </div>
            </Card>
          </div>
        </div>
