...
export default function BrdPage() {
  ...
  const statusCounts = brds.reduce<Record<string, number>>((acc, b) => { acc[b.status] = (acc[b.status] || 0) + 1; return acc; }, {});
  ...
  return (
    <div className="relative h-full w-full min-h-0 bg-slate-50 dark:bg-slate-950 flex flex-col">
                ...
                <tr key={brd.id} className={`transition-colors group ${isSelected ? "bg-blue-50/60 dark:bg-blue-900/10" : "hover:bg-slate-50/80 dark:hover:bg-slate-800/30"}`}>
                  ...
                  <td className="px-3 py-3 whitespace-nowrap">
                    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold ${STATUS_BADGE[brd.status]}`}>
                      <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${STATUS_DOT[brd.status]}`} />{STATUS_LABEL[brd.status]}
                    </span>
                  </td>
                  <td className="px-3 py-3 whitespace-nowrap">
                  ...
                  <td className="px-3 py-3 whitespace-nowrap text-center">
                    <div className="flex items-center justify-center gap-0.5">
                      <button disabled={isLocked} onClick={() => { setFlowFinalMode("view"); setFlowInitialStep(6); setFlowInitialMeta({format:brd.format,brdId:brd.id,title:name}); setShowBrdFlow(true); }} title="View BRD" className="p-1.5 rounded-md text-slate-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 dark:hover:text-blue-400 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"><EyeIcon /></button>
                      <button disabled={isLocked} onClick={() => { setFlowFinalMode("generate"); setFlowInitialStep(6); setFlowInitialMeta({format:brd.format,brdId:brd.id,title:name}); setShowBrdFlow(true); }} title="Edit BRD" className="p-1.5 rounded-md text-slate-400 hover:text-slate-700 hover:bg-slate-100 dark:hover:bg-slate-700 dark:hover:text-slate-200 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"><EditIcon /></button>
                      <button onClick={() => setHistoryBrd(brd)} title="Version History" className="p-1.5 rounded-md text-slate-400 hover:text-violet-600 hover:bg-violet-50 dark:hover:bg-violet-900/20 dark:hover:text-violet-400 transition-colors"><HistoryIcon /></button>
                      <button onClick={() => setDeleteTarget(brd)} title="Remove" className="p-1.5 rounded-md text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 dark:hover:text-red-400 transition-colors"><TrashIcon /></button>
                    </div>
                  </td>
                </tr>
              );
              ...
              <div className="p-6">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 leading-relaxed">
                      This will permanently remove <span className="font-semibold text-slate-700 dark:text-slate-200">{deleteTarget.id}</span> and all associated data. This action cannot be undone.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2.5 mt-5">
                  <button onClick={() => setDeleteTarget(null)} disabled={deleteLoading} className="flex-1 py-2.5 rounded-xl text-xs font-semibold text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 disabled:opacity-50 transition-colors">Cancel</button>
                  <button onClick={confirmDelete} disabled={deleteLoading} className="flex-1 py-2.5 rounded-xl text-xs font-semibold text-white bg-red-600 hover:bg-red-700 active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed transition-all">
                    {deleteLoading ? <span className="flex items-center justify-center gap-1.5"><svg className="w-3.5 h-3.5 animate-spin" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"/></svg>Deleting…</span> : "Delete permanently"}
                  </button>
                </div>
              </div>
            </div>
              ...
              <div className="p-6">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 leading-relaxed">All selected documents and their data will be permanently removed.</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2.5 mt-5">
                  <button onClick={() => setBulkDeleteOpen(false)} disabled={deleteLoading} className="flex-1 py-2.5 rounded-xl text-xs font-semibold text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 disabled:opacity-50 transition-colors">Cancel</button>
                  <button onClick={confirmBulkDelete} disabled={deleteLoading} className="flex-1 py-2.5 rounded-xl text-xs font-semibold text-white bg-red-600 hover:bg-red-700 active:scale-[0.98] disabled:opacity-60 transition-all">
                    {deleteLoading ? <span className="flex items-center justify-center gap-1.5"><svg className="w-3.5 h-3.5 animate-spin" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"/></svg>Deleting…</span> : `Delete ${selected.size} permanently`}
                  </button>
                </div>
              </div>
            </div>
      ...
      {/* ── Version History Modal ── */}
      {historyBrd && <VersionHistoryModal brd={historyBrd} onClose={() => setHistoryBrd(null)} />}
    </div>
  );
}