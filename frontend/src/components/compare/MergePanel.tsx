...
interface MergePanelProps {
  /** Active job from page — enables auto-loading chunks via GET /compare/chunks */
  activeJob?: { job_id: string; source_name: string; status: string } | null;
}

export default function MergePanel({ activeJob }: MergePanelProps = {}) {
  ...
  // Auto-load chunks from job when activeJob is done
  React.useEffect(() => {
    if (!activeJob?.job_id || activeJob.status !== "done") return;
    setSourceName(activeJob.source_name);

    async function loadChunks() {
      setLoadingChunks(true);
      try {
        const res = await fetch(
          `${PROCESSING_URL}/compare/chunks?job_id=${encodeURIComponent(activeJob!.job_id)}`,
        );
        if (!res.ok) return;
        const data = await res.json();
        const loaded: ChunkFile[] = (data.chunks ?? []).map(
          (c: { index: number; filename: string; has_changes: boolean }) => ({
            id: String(c.index),
            filename: c.filename,
            xml_content: "", // loaded on demand
            has_changes: c.has_changes,
            validated: false,
            validation_errors: [],
          }),
        );
        if (loaded.length > 0) setChunks(loaded);
      } catch {
  ...
  function handleChunkFilesAdded(files: FileList | null) {
    ...
    Array.from(files).forEach((file) => {
      ...
      reader.onload = (e) => {
        ...
        // Use DOMParser for reliable XML syntax validation
        let isValid = false;
        ...
        try {
          ...
          isValid = false;
          validationErrors = ["File does not appear to be valid XML"];
        }
        newChunks.push({
        ...
        if (newChunks.length === files.length) {
          setChunks((prev) => {
            ...
            return allChunks.sort(
              (a, b) =>
                parseChunkIndex(a.filename) - parseChunkIndex(b.filename),
            );
          });
        }
      };
          ...
          <div className="w-[320px] flex-shrink-0 flex flex-col gap-3">
            ...
            <div className="rounded-xl border border-slate-700/50 bg-slate-900/40 p-3">
              <div className="flex items-center justify-between mb-2">
                ...
                <input
                  ...
                  className="hidden"
                  onChange={(e) => {
                    handleChunkFilesAdded(e.target.files);
                    e.target.value = "";
                  }}
                />
              </div>
              ...
              <div
                ...
                onDrop={(e) => {
                  e.preventDefault();
                  handleChunkFilesAdded(e.dataTransfer.files);
                }}
              ...
              >
                <p className="text-xs text-slate-600">
                  Drop XML chunks here or click Add Files
                </p>
                <p className="text-[10px] text-slate-700 mt-0.5">
                  Accepts: SourceName_innod.NNNNN.xml
                </p>
              </div>
            ...
            {missingChunks.length > 0 && (
              <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 px-3 py-2.5">
                ...
                <p className="text-[11px] text-amber-400/80">
                  Indices {missingChunks.join(", ")} appear to be missing from
                  the sequence.
                </p>
              </div>
            )}
              ...
              <div className="rounded-xl border border-slate-700/40 bg-slate-900/30 px-3 py-2.5">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] text-emerald-400">
                      {chunks.length - invalidChunks.length} valid
                    </span>
                    {invalidChunks.length > 0 && (
                      <span className="text-[10px] text-red-400">
                        {invalidChunks.length} invalid
                      </span>
                    )}
                  </div>
                </div>
                <div className="mt-1.5 h-1.5 bg-slate-800 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${chunks.length ? ((chunks.length - invalidChunks.length) / chunks.length) * 100 : 0}%`,
                    }}
                  />
                </div>
              </div>
            ...
            {!sourceName.trim() && chunks.length > 0 && (
              <p className="text-[10px] text-slate-600 text-center">
                Enter a source name to enable merge
              </p>
            )}
            ...
            <div className="flex-1 rounded-xl border border-slate-700/50 bg-slate-900/30 overflow-hidden flex flex-col">
              <div className="flex-shrink-0 flex items-center justify-between px-4 py-2.5 border-b border-slate-800">
                ...
                {mergeResult && (
                  <button
                    onClick={() =>
                      downloadBlob(mergeResult.merged_xml, mergeResult.filename)
                    }
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-center gap-4 p-6">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-600 mt-1 max-w-sm">
                      Upload XML chunk files (from CHUNKED folder), enter a
                      source name, then click{" "}
                    ...
                    {chunks.length === 0 && (
                      <div className="mt-4 grid grid-cols-3 gap-2 max-w-sm mx-auto text-left">
                        {[
                          ...
                          <div
                            key={step}
                            className="p-2 rounded-lg border border-slate-800 bg-slate-900/40"
                          >
                            <div className="w-5 h-5 rounded-full bg-slate-700 flex items-center justify-center text-[10px] font-bold text-slate-400 mb-1">
                              {step}
                            </div>
                ...
                <div key={label} className="flex items-center gap-2 min-w-0">
                  ...
                  {file ? (
                    <div className="flex items-center gap-1 min-w-0">
                      <span
                        className={`w-1.5 h-1.5 rounded-full shrink-0 ${dotMap[color]}`}
                      />
                      <span className="text-xs text-slate-300 truncate max-w-[100px] font-mono">
                        {file.name}
                      </span>
                      <button
                        onClick={() => setFiles[i](null)}
                  ...
                  <input
                    ...
                    className="hidden"
                    onChange={(e) => {
                      setFiles[i](e.target.files?.[0] ?? null);
                      e.target.value = "";
                    }}
                  />
                </div>
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-600 mt-1 max-w-xs">
                    Select OLD PDF, NEW PDF, and XML file, load their
                    differences, then accept or reject each change.
                  </p>
                </div>
              </div>
            )
