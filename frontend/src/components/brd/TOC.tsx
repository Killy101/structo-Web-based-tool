...
function buildRowsFromToc(initialData?: Props["initialData"]): TocRow[] {
  ...
  return sections
    ...
    .map((section, index) => {
      // Handle the case where level might be a string like "**10**" with markdown
      let level = String(section.level || section.id || index + 1);
      
      // Extract just the number if it has markdown formatting (supports multi-digit)
      const levelMatch = level.match(/\*\*?(\d+)\*\*?|\b(\d+)\b/);
      if (levelMatch) {
        level = levelMatch[1] || levelMatch[2] || level;
      }
    ...
    .sort((a, b) => {
      // Sort by level numerically (supports multi-digit)
...
function mapRequiredValue(val?: string): TocRow["required"] {
  ...
  // Try to check if it's a valid enum value
...
const COLUMNS = [
  { key: "level",           label: "Level",            width: "w-20",  icon: "⬡" }, // Increased width for two digits
...
function levelBadge(val: string) {
  // Support for multi-digit levels (0-99)
  ...
  // For levels beyond 15, use a default style
...
export default function TOC({ initialData }: Props) {
  ...
  function addRow() {
    // Find the highest level number to suggest next
    const maxLevel = rows.reduce((max, row) => {
      const levelNum = parseInt(row.level) || 0;
