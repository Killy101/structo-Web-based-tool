...
export default function Citation() {
  ...
  function addRow() {
    const newRow: CitationRow = { id: Date.now().toString(), level: "1", citationRules: "", sourceOfLaw: "", smeComments: "" };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-black dark:text-slate-300" style={{ fontFamily: "'DM Mono', monospace" }}>Citation</p>
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse" style={{ minWidth: "720px" }}>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-[#2a3147]" style={{ fontWeight: 400 }}>
              ...
              {rows.length === 0 && (
                <tr><td colSpan={COLUMNS.length + 1} className="py-12 text-center"><p className="text-[12.5px] text-slate-500 dark:text-slate-500">No citation rules yet</p><button onClick={addRow} className="mt-2 text-[12px] text-blue-600 dark:text-blue-400 hover:underline">+ Add first rule</button></td></tr>
              )}
            </tbody>
          </table>
        </div>
